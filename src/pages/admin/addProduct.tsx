...
const AddProduct: React.FC = () => {
    ...
    const handleImageChange = (
        type: "main" | "hover" | "product",
        files: FileList | null
    ) => {
        if (type === "main" || type === "hover") {
            setImages((prev) => ({
                ...prev,
                [type]: files?.[0] || null,
            }));
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (images.main) formData.append("mainImage", images.main);
        if (images.hover) formData.append("hoverImage", images.hover);
                            ...
                            <div className="bg-white p-8 shadow-lg">
                                ...
                                <form onSubmit={handleSubmit} className="space-y-6">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                        ...
                                        <div>
                                            <label htmlFor="hoverImage" className="block text-sm font-medium text-gray-700 mb-1">
                                                Ảnh hover
                                            </label>
                                            <input
                                                type="file"
                                                id="hoverImage"
                                                onChange={(e) => handleImageChange("hover", e.target.files)}
                                                accept="image/*"
                                                className="w-full p-2 border border-gray-300 rounded-md shadow-sm"
                                            />
                                        </div>
                                        <div>
                                            ...
                                            <input
                                                type="file"
                                                id="productImages"
                                                onChange={(e) => handleImageChange("product", e.target.files)}
                                                accept="image/*"
                                                multiple
                                                className="w-full p-2 border border-gray-300 rounded-md shadow-sm"
                                            />
